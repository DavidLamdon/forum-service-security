...
import telran.java2022.accounting.dto.exceptions.UserPasswordExpiredExeption;
...
public class UserAccountServiceImpl implements UserAccountService, CommandLineRunner {
	...
	public UserAccountResponseDto getUser(String login) {
		...
		if(userAccount.getTimeOfCreation().isEqual(userAccount.getTimeOfExpiration())) {
			throw new UserPasswordExpiredExeption();
		}
		return modelMapper.map(userAccount, UserAccountResponseDto.class);
	}
	...
	public void changePassword(String login, String newPassword) {
		UserAccount userAccount = repository.findById(login).orElseThrow(() -> new UserNotFoundException());
		userAccount.setTimeOfCreation(LocalDateTime.now());
		userAccount.setTimeOfExpiration(userAccount.getTimeOfCreation().plusDays(64));
